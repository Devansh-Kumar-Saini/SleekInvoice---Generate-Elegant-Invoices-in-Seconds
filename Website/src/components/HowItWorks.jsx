...
export default function HowItWorks() {
  return (
    <section id="how-it-works" style={{ scrollMarginTop: 84, position: 'relative', zIndex: 1, maxWidth: 1120, margin: '0 auto', padding: '100px 24px' }}>
      ...
      <div className="sv-reveal sv-d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 24 }}>
        {HOW_IT_WORKS.map((step) => (
          <div key={step.num} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-hair)', borderRadius: 18, padding: 32 }}>
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 34, fontWeight: 700, color: 'var(--amber-ink)', opacity: 0.85, marginBottom: 14 }}>{step.num}</div>
