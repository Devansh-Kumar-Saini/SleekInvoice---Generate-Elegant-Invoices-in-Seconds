...
export default function Testimonials() {
  return (
    <section style={{ position: 'relative', zIndex: 1, maxWidth: 1280, margin: '0 auto', padding: '100px 24px' }}>
      ...
      <div className="sv-reveal sv-d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20 }}>
        {TESTIMONIALS.map((t) => (
          <div key={t.initials} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-hair)', borderRadius: 18, padding: 30 }}>
            <div style={{ display: 'flex', gap: 3, color: 'var(--amber-ink)', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 18, borderTop: '1px solid var(--border-hair)' }}>
              <span style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--bg-surface-2)', border: '1px solid var(--border-hair-strong)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600, color: 'var(--amber-ink)', flexShrink: 0 }}>
                {t.initials}
              </span>
              <div>
